// Archived legacy implementation of Restapify (2025-10-20)
// -------------------------------------------------------
// The content below is a verbatim copy from the pre-refactor `src/Restapify.ts`.
// It is retained for historical and reference purposes only.
// New development should occur in `packages/core-lib` and service/CLI layers.
      ...
      (async () => {
        ...
        try {
          if(this.useLocal) {
            ...
            this.proxyBaseUrl = 'https://localhost:44319';
            console.log(`Using local proxy base URL: ${this.proxyBaseUrl}`);
          ...
          if (this.proxyBaseUrl != '') {
            ...
            try {
              ...
              if (req.originalUrl.startsWith('/api/settings/public')) {
                proxyData = { epiSettings: proxyData };
              }
            } catch (jsonError) {
              console.error(
