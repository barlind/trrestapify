...
var getFiles = function getFiles(p) {
  return fs.readdirSync(p).filter(function (f) {
    return fs.statSync(path.join(p, f)).isFile();
  });
};
...
var _getRouteFiles = function getRouteFiles(rootDir) {
  ...
  dirNames.forEach(function (dir) {
    return _getRouteFiles(path.resolve(rootDir, dir), files);
  });
  return files;
};
...
var Restapify = /*#__PURE__*/_createClass(function Restapify(_ref) {
  ...
  _defineProperty(this, "configHotWatch", function () {
    if (_this.hotWatch) {
      _this.chokidarWatcher = chokidar.watch(_this.rootDir, {
        ignoreInitial: true
      });
  ...
  _defineProperty(this, "checkRootDirectory", function () {
    var folderExists = fs.existsSync(_this.rootDir);
    if (!folderExists) {
        ...
        return _regeneratorRuntime.wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            case 0:
              ...
              customFilePath = path.join(routeData.directoryPath, customJsonFilename);
              if (!fs.existsSync(customFilePath)) {
                _context2.next = 11;
                break;
              }
              // 3. Serve that file’s content
              customFileContent = JSON.parse(fs.readFileSync(customFilePath, 'utf8'));
...
export { Restapify as Routes, cli, Restapify as default };
